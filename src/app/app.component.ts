...
export class AppComponent {
  ...
  generar(){
    ...
    if(this.NNodos <= 2){
      ...
      for(let i = 0; i < this.NNodos; i++){
        ...
        nodo.bgcolor = '#000';
        nodo.top = 20 + (this.wHeight / (this.NNodos * 1.2) * (Math.random() * this.NNodos + 1));
        nodo.left = this.wWidth / (this.NNodos * 1.2) * (Math.random() * this.NNodos + 1);
        nodo.border = 2;
        nodo.value = environment.chars[i];
        nodo.font = nodo.size / 4;
        nodo.padding = 8;
        nodo.posicion = i;
  ...
  recorridoK(anteriores: number[], actual: number): number{
    let sum: number = 1;
    for(let i = 0; i < this.aristas.length; i++){
      if((this.aristas[i].relacion[0] == actual || this.aristas[i].relacion[1] == actual)){
